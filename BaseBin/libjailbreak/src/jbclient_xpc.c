...
int jbclient_platform_clear_process_noattach(uint64_t pid, bool preflight, bool hideTraced)
{
    ...
    xpc_dictionary_set_bool(xargs, "preflight", preflight);
    xpc_dictionary_set_bool(xargs, "hide-traced", hideTraced);
    xpc_object_t xreply = jbserver_xpc_send(JBS_DOMAIN_PLATFORM, JBS_PLATFORM_CLEAR_PROCESS_NOATTACH, xargs);
    xpc_release(xargs);
    if (xreply) {
        int result = xpc_dictionary_get_int64(xreply, "result");
        xpc_release(xreply);
